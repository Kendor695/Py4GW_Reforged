...
def _count_earshot_enemies(player_id: int) -> int:
    """Count alive enemies actually within Earshot range (GetEnemyArray() has no range filter)."""
    px, py = Agent.GetXY(player_id)
    count = 0
    for enemy_id in AgentArray.GetEnemyArray():
        if not Agent.IsAlive(enemy_id):
        ...
        ex, ey = Agent.GetXY(enemy_id)
        dx, dy = ex - px, ey - py
        if dx * dx + dy * dy <= _EARSHOT_SQ:
            count += 1
    return count


def _has_dead_party_member_nearby(player_id: int) -> bool:
    """Return True if any hero OR other party player (e.g. a multibox alt) is dead within Nearby range."""
    px, py = Agent.GetXY(player_id)
    ...
    for player in GLOBAL_CACHE.Party.GetPlayers():
        pid = GLOBAL_CACHE.Party.Players.GetAgentIDByLoginNumber(player.login_number)
        if pid <= 0 or pid == player_id or Agent.IsAlive(pid):
            continue
        ax, ay = Agent.GetXY(pid)
        dx, dy = ax - px, ay - py
        if dx * dx + dy * dy <= _NEARBY_SQ:
            return True
...
class JununduWurm(BuildMgr):
    ...
    def _process_combat(self):
        ...
        if _combat_log_timer.IsExpired():
            ...
            enemy_count = _count_earshot_enemies(player_id)
            px, py = Agent.GetXY(player_id)
            PySystem.Console.Log(_LOG,
        ...
        # Wail — dead teammate (hero or other account) nearby; use as revive-window armor buff
        if _has_dead_party_member_nearby(player_id) and (yield from self.CastSkillSlot(7, aftercast_delay=500)):  # Wail
            return
    ...
    def _process_ooc(self):
        ...
        uptime the bot script's own movement steps give the leader, but the standard
        HeroAI widget driving alts never picked it up in practice, and the leader
        moving faster than alts while alts stood still on Tunnel widened the gap
